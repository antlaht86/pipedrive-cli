...
const run = (fixtures: readonly Fixture[], argv: readonly string[] = []): Promise<Run> =>
  runWith(fixtures, ["deals", "list", ...argv]);
...
describe("--limit is a bound: exit 0 and a summary", () => {
  test("a limit that cuts a page short emits exactly that many records", async () => {
    const { exit, lines, last } = await run([page(deals(1, 10), "c2")], [
      "--limit",
      "4",
    ]);
  ...
  test("a limit larger than the data is not a bound at all", async () => {
    const { exit, last } = await run([page(deals(1, 3), null)], ["--limit", "100"]);
  ...
  test("a limit filling exactly at a page boundary with a null cursor is complete", async () => {
    const { exit, last } = await run([page(deals(1, 2), null)], ["--limit", "2"]);
  ...
  test("a limit filling where the cursor continues reports complete: false", async () => {
    const { exit, last } = await run([page(deals(1, 2), "c2")], ["--limit", "2"]);

    expect(exit).toBe(0);
    expect(last).toMatchObject({ complete: false, emitted: 2, reason: "limit" });
...
describe("--max-requests is a guard: exit 3 and an error", () => {
  ...
  test("a run that fits under the ceiling is an ordinary success", async () => {
    const { exit, last } = await run([page(deals(1, 2), null)], [
      "--max-requests",
      "9",
    ]);

    expect(exit).toBe(0);
    expect(last).toMatchObject({ type: "summary", complete: true, requests: 1 });
  });
  ...
  test("retries count against the headroom", async () => {
    ...
    const { exit, last } = await run([failure, failure, page(deals(1, 1), null)], [
      "--max-requests",
      "2",
    ]);
  ...
  test("the guard wins over the bound when both are reachable", async () => {
    const { exit, last } = await run(many(), ["--limit", "1000", "--max-requests", "2"]);
  ...
  test("the bound wins when it fills first", async () => {
    const { exit, last } = await run(many(), ["--limit", "2", "--max-requests", "9"]);
...
describe("--limit does not exist on a non-list command", () => {
  ...
  test("the get refusal still names the flags get does take", async () => {
    ...
    expect(last["message"]).toBe(
      "pd deals get does not accept --limit. It takes --token-file and --max-requests and no other flag.",
    );
  });
});
...
describe("both flags reject their bad values offline", () => {
  ...
  for (const flag of ["--limit", "--max-requests"]) {
    for (const value of ["0", "-1", "1.5", "abc", "", " 4", "4 ", "1e3", "0x10"]) {
      test(`${flag} ${JSON.stringify(value)} is usage, exit 2, and no request`, async () => {
        ...
        expect(exit).toBe(2);
        expect(lines).toHaveLength(1);
        expect(last).toMatchObject({ code: "usage", exit_code: 2, requests: 0 });
  ...
  test("a flag with no value at all is a usage error", async () => {
    const { exit, last } = await runWith(undefined, ["deals", "list", "--limit"]);
