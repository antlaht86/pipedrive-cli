...
let home = "";
beforeEach(() => {
	home = mkdtempSync(`${tmpdir()}/pd-resolve-variable-`);
});
afterEach(() => {
...
const organizationBatch = (ids: readonly number[]): Fixture => ({
	...
	body: {
		success: true,
		data: ids.map((id) => organization(id)),
		additional_data: { next_cursor: null },
	},
});
...
describe("--resolve variable-cost enrichment", () => {
	test("resolves standard and custom person relations in page-local batches of 100 with a run map", async () => {
		...
		const firstPage = firstIds.map((id, index) =>
			deal(index + 1, {
				person_id: id,
		...
		const { exit, lines, last } = await run(
			[
				...
				{
					path: DEALS_PATH,
					query: { ...dealsQuery(), custom_fields: PERSON_HASH },
					body: dealsPage(firstPage, "c2"),
				},
				personBatch(firstIds.slice(0, 100)),
				personBatch(firstIds.slice(100)),
				{
					path: DEALS_PATH,
					query: { ...dealsQuery("c2"), custom_fields: PERSON_HASH },
					body: dealsPage(secondPage, null),
				},
				personBatch([6_000]),
			],
	...
	test("emits the first resolved page in roughly 250 ms before requesting the next page", async () => {
		...
		const replay = createReplayTransport([
			{
				path: DEALS_PATH,
				query: dealsQuery(),
				body: dealsPage([deal(1, { person_id: 8_201 })], "c2"),
			},
			personBatch([8_201]),
			{
				path: DEALS_PATH,
				query: dealsQuery("c2"),
				body: dealsPage([deal(2, { person_id: 8_202 })], null),
			},
			personBatch([8_202]),
		]);
		const out = capture();
		const exit = await route({
			...
			transport: async (request) => {
				events.push(
					request.url.includes("cursor=c2") ? "request:c2" : "request:first",
				);
				await Bun.sleep(100);
		...
		expect(exit).toBe(0);
		expect(events.indexOf("record:1")).toBeLessThan(
			events.indexOf("request:c2"),
		);
		expect(
			(firstRecordAt ?? Number.POSITIVE_INFINITY) - startedAt,
		).toBeLessThan(350);
	});
	...
	test("yields relation enrichment before consuming the last --max-requests slot", async () => {
			...
			[
				{
					path: DEALS_PATH,
					query: dealsQuery(),
					body: dealsPage([deal(1, { person_id: 8_001 })], null),
				},
			],
		...
		expect(
			warnings(lines).filter(
				(line) => line.kind === "resolution_budget_exhausted",
			),
		).toHaveLength(1);
			...
			[
				{
					path: DEALS_PATH,
					query: dealsQuery(),
					body: dealsPage([deal(1, { person_id: id })], null),
				},
				failed,
				failed,
				personBatch([id]),
			],
			["--fields", "person_id", "--resolve-budget", "2"],
			new FakeClock(),
		);
			...
			[
				{
					path: DEALS_PATH,
					query: dealsQuery(),
					body: dealsPage([deal(1, { person_id: id })], "c2"),
				},
				failed,
				failed,
				personBatch([id]),
				{
					path: DEALS_PATH,
					query: dealsQuery("c2"),
					body: dealsPage([deal(2, { person_id: id + 1 })], null),
				},
			],
			["--fields", "person_id", "--max-requests", "4"],
			new FakeClock(),
		);
	...
	test("uses a default relation ceiling of 50 requests", async () => {
		...
		expect(
			out
				.of("warning")
				.filter((line) => line.kind === "resolution_budget_exhausted"),
		).toHaveLength(1);
