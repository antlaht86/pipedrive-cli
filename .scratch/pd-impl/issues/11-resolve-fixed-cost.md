...
- [ ] `--resolve` adds all seven `*_name` siblings where resolvable, additively
- [ ] Raw output is byte-identical with and without the flag, `custom_fields` included
- [ ] An unresolvable id omits its sibling key entirely
- [ ] `custom_fields_resolved` is keyed by hash and carries `name` plus `label` where meaningful
- [ ] Two custom fields sharing a display name both resolve without collision
- [ ] Money resolves as `"12000.00 EUR"` with no locale formatting, identically on any machine
- [ ] `include_option_labels` is never sent
- [ ] A cold cache costs at most four requests; a warm cache costs zero
- [ ] Any resolution failure degrades: one warning, raw ids onward, `resolved: "partial"`, exit 0
- [ ] `resolved` is `"off"` / `"partial"` / `"full"` on every trailer
- [ ] An unrecognised hash forces exactly one schema refresh per schema per run, then one `unknown_custom_field` warning
