...
- `generate.ts` writes `samples/`: the same result set in both candidate formats, across six cases.
- `bench.ts` measures what buffering costs on a 40,000-deal account.

## No longer throwaway: the `.ndjson` files are generated by the shipped writer

Ticket 05 stopped transcribing these files by hand. Every `*.ndjson` sample is now produced by
driving `src/lib/output/ndjson-writer.ts` — the same object that writes `pd`'s stdout — and
`test/output-samples.test.ts` re-runs the cases and compares the committed files byte for byte.

That is what makes ADR-0002's "only guard against drift" mechanical rather than aspirational. A
failing comparison is one of two things and never a third: the writer changed by accident, or the
format changed on purpose and the samples were not regenerated in the same commit.

The `*.array.json` files stay hand-written, because nothing produces them and nothing ever will.
...
| E validation warning | `e-validation-warning.*` | One record fails zod, the run continues; the `warning` carries `kind`, `resource`, `path` and `issue` (ADR-0006 §6) |
