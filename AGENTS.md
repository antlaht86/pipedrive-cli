...
A user record carries `is_global_admin` and `is_deal_admin`. Both are always present and always a boolean. `is_global_admin` is the `admin` flag of the `access` entry whose `app` is `global`; `is_deal_admin` is the one whose `app` is `sales`, which Pipedrive's UI calls the deal admin. Pipedrive omits an entry instead of sending `admin: false`, so an absent entry reads as `false`. A user with no `access` at all reads as `false` for both. Read these fields rather than searching `access` yourself. The raw `access` array is not emitted by default; ask for it with `--fields access` when you need the per-app `permission_set_id`. `pd users list --admin global` emits only the global admins and `pd users list --admin deal` only the deal admins; any other value is refused. The flag exists on `list` alone, and it filters the whole list before `--limit` bounds it.
